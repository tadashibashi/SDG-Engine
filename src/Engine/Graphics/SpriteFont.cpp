...
namespace SDG {
    ...
    void SpriteFont::draw(SpriteBatch& batch, const char* s, glm::vec2 position, glm::vec2 scaling,
        ...
        for (int si = 0; s[si] != 0; si++) {
            char c = s[si];
            if (s[si] == '\n') {
                tp.y += (float)_fontHeight * scaling.y;
                tp.x = position.x;
            }
            else {
                // Check for correct glyph
                ...
                FRectangle dest(tp.x, tp.y, _glyphs[gi].size.x * scaling.x, _glyphs[gi].size.y * scaling.y);
                FRectangle source(_glyphs[gi].uvRect.x, _glyphs[gi].uvRect.y, _glyphs[gi].uvRect.z, _glyphs[gi].uvRect.w);
                //if (c == '\u00E4') std::cout << _glyphs[gi].uvRect.x << " " << _glyphs[gi].uvRect.y << " " << _glyphs[gi].uvRect.z << " " << _glyphs[gi].uvRect.w << std::endl;
                batch.DrawTexture(SDG::Texture2D(_texID), dest, source, tint, depth);
